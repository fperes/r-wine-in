...
# Data               : 2017-06-09
# Arquivo            : Análise multi variada com cor do vinho / $color
#
#*******************************************************************************
...
ind <- sample(2, nrow(all.wine), replace = TRUE, prob = c(0.8, 0.20))
...
ctree.Predict <- predict(wine.ctree, newdata = testData)
(ctree.test.table <- table(ctree.Predict, testData$color))

# plot confusion matrix results based on test set
par.customized <- par(mfrow = c(1, 2))
plot.model.summary.by.color(t = ctree.test.table, title = "CTREE - Pelo conjunto de TESTE")
...
#*******************************************************************************
# 6. train: BAGGING CART (Training set x Test Set)
# Faz a divisão da árvore com base nos valores que minimizam uma funçãoo de custo,
# como soma dos mínimos quadrados.
#*******************************************************************************
...
#*******************************************************************************
# 7. train: C5.0 (Training set x Test Set)
# Faz a divisão da árvore com base nos valores que minimizam uma funçãoo de custo,
# como soma dos mínimos quadrados.
#*******************************************************************************
